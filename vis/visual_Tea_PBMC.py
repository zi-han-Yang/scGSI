...
path = "E:/experiment/scGSI/"
dataset_dir = path + 'data/'
result_dir = path + "results/" + dataset_name
eva_dir = path + "eva/" + dataset_name
vis_dir = path + 'vis/' + dataset_name

methods = ['raw_data', 'JointMDS', 'MMDMA', 'Pamona', 'SCOT', 'UnionCom', 'scTopoGAN', 'scMGCL', 'MultiVI', 'scPairing', 'scGSI']
methods1 = ['scGSI']
...
adata, anno_rna, anno_atac, clu_rna, clu_atac = load_result(dataset_name, dataset_dir, result_dir,
                                                            methods)
adata.obs_names_make_unique()  # 确保观察名称唯一
...
eva_metrics1 = pd.read_csv(eva_dir + '/eva_metrics_' + dataset_name + '.csv', index_col=0, header=0)
eva_metrics = pd.read_csv(eva_dir + '/eva_metrics_' + dataset_name + '_Ablation' + '.csv', index_col=0, header=0)
...
accuracy = {}
CM = {}  # 混淆矩阵
cluster_labels_np = np.concatenate([clu_rna, clu_atac], axis=0)  # 合并聚类标签
